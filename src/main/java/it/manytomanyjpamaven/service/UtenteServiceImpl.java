...
public class UtenteServiceImpl implements UtenteService {
	...
	public void rimuovi(Long idUtente) throws Exception {
		...
		// deve rimuovere un utente
		// Carichiamo l utente con l eager

		// Lanciamo un eccezione custom con il nome UtenteConRUoiliAssociatiException
		...
		try {
			...
			// uso l'injection per il dao
			utenteDAO.setEntityManager(entityManager);
			// Ci prendiamo l utente con l eager
			Utente daRimuovere = utenteDAO.findByIdFetchingRuoli(idUtente);

			// Controlliamo se ha elementi nel suo arrayLIst di ruoli
			if (daRimuovere.getRuoli().size() > 0)
				throw new UtenteConRuoliAssociatiException("ERRORE: Utente con ruoli ancora associati");

			// Chiamialo il metodo delete
			utenteDAO.delete(daRimuovere);
	...
	public List<Utente> trovaTuttiConDataCreazineAGiugno() {
		...
		try {
			...
			// eseguo quello che realmente devo fare
			return utenteDAO.findAllByDateCreatedUgualeAGiugno();
	...
	public List<Utente> trovaUtentiConPasswordMinoreDiOttoCaratteri() {
		...
		try {
			...
			// eseguo quello che realmente devo fare
			return utenteDAO.findAllByPasswordShorterThanEightChar();
